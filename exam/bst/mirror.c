...
int main(void) {
    ...
    //Per avere il test superato, creo l'albero che sara' lo specchio di albero e stampo lui
    BST *specchio;
    mirror(albero, &specchio);
    print_rec(specchio);
    //print_bst2D(albero);
...
//Pre:ptrPtr puntatore di puntatore a nodo radice, anche vuoto, val intero valido
//Post:fa l'inserimento di un nuovo nodo secondo le regole BST, ovvero sara' a sinistra di ogni nodo genitore dal valore >=al figlio e a destra di ogni nodo genitore con valore<figlio
void ordinsert_rec(BST **ptrPtr, int val) {
    if(*ptrPtr==NULL)
    {
    ...
    } else if(val<=(*ptrPtr)->valore) ordinsert_rec(&((*ptrPtr)->leftPtr),val);
    else ordinsert_rec(&((*ptrPtr)->rightPtr), val);
}
