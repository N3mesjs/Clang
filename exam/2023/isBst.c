...
int isValueInRange(BST *ptr, int min, int max){
    if(ptr == NULL) return 1;

    if(ptr->valore <= min || ptr->valore > max) return 0;

    return isValueInRange(ptr->leftPtr, min, ptr->valore) &&
           isValueInRange(ptr->rightPtr, ptr->valore, max);
}

// restituisce 1 se l'albero è un BST, 0 altrimenti
int isbst(BST *ptr) {
    return isValueInRange(ptr, INT_MIN, INT_MAX);
}
