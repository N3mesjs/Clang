 ...
 * {1,0,X,0,0}
 * 
 * La funzione ha firma int mossa(int dim_x, int dim_y, int campo[dim_x][dim_y], int pos_x, int pos_y), 
 * e deve rispettare le seguenti PRE e POST:
...
int mossa(int dim_x, int dim_y, int campo[dim_x][dim_y], int pos_x, int pos_y){
    if(pos_x < 0 || pos_x >= dim_x || pos_y < 0 || pos_y >= dim_y) return 0;
    if(campo[pos_x][pos_y] == 0) return 0;
    if(pos_x == 0) return 1;
